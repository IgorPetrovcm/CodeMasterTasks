...
app.MapGet(
    ...
    (int gateId, [FromServices] GatesManager gatesManager) => {
            gatesManager.RequestOpen(gateId);
        });

app.MapGet(
    "/api/request/close/{gateId:int}",
    (int gateId, [FromServices] GatesManager gatesManager) => {
            gatesManager.RequestClose(gateId);
    } );
...
app.MapGet(
    ...
    (int gateId, [FromServices] GatesManager gatesManager) => {
            gatesManager.RegisterVisitorEnter(gateId);
        });

app.MapGet(
    "/api/leave/{gateId:int}",
    (int gateId, [FromServices] GatesManager gatesManager) => {
            gatesManager.RegisterVisitorLeave(gateId);
        });
